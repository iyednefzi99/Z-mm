...
i18n by XML file & \texttt{messages\_fr/\allowbreak en/\allowbreak ar\allowbreak .properties} + \texttt{MessageSource} & Same principle of text externalisation, standard \texttt{.properties} format, still FR / EN / AR. \\
\texttt{ConfigZumm.ini} & \texttt{application.yml} + \texttt{@Configuration\allowbreak Properties} & Parameterisation without recompilation, injectable per profile. The thresholds and units become typed beans. \\
...
XML API web service \texttt{getZumm\allowbreak Honey\allowbreak Actual\allowbreak Quantity} & REST JSON API (\texttt{@GetMapping}) + OpenAPI / Swagger & JSON instead of XML, auto-generated documentation. XML remains possible via \texttt{produces = APPLICATION\_XML}. \\
