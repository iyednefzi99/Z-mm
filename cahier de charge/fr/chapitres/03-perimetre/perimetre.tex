...
Plusieurs perspectives initialement envisagées ont été \emph{concrétisées} au fil des
sprints et font désormais partie du produit livré : le module de production et de
récolte, la traçabilité du miel par lot et QR code (anticipant la directive EU
Petit-Déjeuner 2026), le contexte météorologique, la carte des ruchers avec rayons
de butinage, l'ingestion de mesures de capteurs (REST/MQTT) et la détection
d'anomalie sur les séries de poids. Un dernier incrément a ajouté les notifications
d'alerte par e-mail, le rapport de visite en PDF, le journal d'audit et la prévision
de récolte.

Au-delà du périmètre livré, les évolutions suivantes pourront être étudiées :
\begin{itemize}
  ...
  \item \textbf{Cartographie spatiale avancée (PostGIS).}
  La carte des ruchers et les rayons de butinage sont livrés. Restent à exploiter
  pleinement la base spatiale PostGIS : clustering des ruchers, distances
  inter-ruchers, optimisation des routes de visite et cartographie florale par
  satellite --- un avantage concurrentiel inédit chez les plateformes existantes.

  \item \textbf{Connecteur IoT standardisé et durci.}
  L'ingestion REST/MQTT des mesures est livrée. Un endpoint de télémétrie dédié
  (\texttt{/api/v1/telemetry}) avec authentification par token \emph{device} et
  schéma de données normalisé (poids, température, humidité, acoustique) permettrait
  d'intégrer les capteurs du marché (BroodMinder, ApisProtect, BEEP, ESP32 DIY)
  sans développer de matériel.

  \item \textbf{Détection acoustique et vision par ordinateur.}
  La détection d'anomalie statistique (EWMA) est livrée ; l'étape suivante relève de
  l'apprentissage automatique. Les recherches récentes montrent une précision de 85 à
  99\,\% pour la détection acoustique du varroa et de l'essaimage, ainsi que le
  comptage automatisé de varroa sur planche collante par vision (YOLOv11 Nano,
  $R^2 = 0{,}99$). Ces modèles open-source pourraient être intégrés dans un module
  d'inspection photo et un analyseur acoustique, enrichissant les rapports de visite
