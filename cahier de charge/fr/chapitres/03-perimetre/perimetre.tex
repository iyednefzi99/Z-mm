...
\begin{itemize}
  ...
  \item \textbf{Module production et récolte.}
  Le suivi des récoltes (pesée, rendement, classification), central dans la gestion
  apicole, est absent du périmètre actuel. Ce module permettrait d'enregistrer les
  récoltes par ruche et par saison, de calculer les rendements, de suivre la qualité
  du miel (pollen, humidité, conductivité) et de produire des rapports de production
  pour les subventions et le contrôle sanitaire.

  \item \textbf{Traçabilité du miel --- Directive EU Petit-Déjeuner (2026).}
  La directive européenne impose la traçabilité complète du miel d'ici juin 2026 :
  numérotation des lots, historique des traitements phytosanitaires, certifications
  (bio, IGP, AOP), QR code sur le pot relié à la fiche ruche. Aucune plateforme
  concurrente n'offre de module intégré de traçabilité, ce qui représente un
  différenciateur majeur pour Zümm.

  \item \textbf{Intégration des données météorologiques.}
  La température, l'humidité, le vent et l'ensoleillement impactent directement le
  comportement des colonies (essaimage, butinage, consommation de provisions). Une
  corrélation des données météo avec les capteurs de ruche permettrait des alertes
  contextualisées et une meilleure planification des visites.

  \item \textbf{Visualisation cartographique PostGIS.}
  Le backend repose sur PostgreSQL/PostGIS, une capacité inédite chez les
  concurrents. Exploiter cette base spatiale pour une carte interactive des ruchers
  (clustering, distances inter-ruchers, routes de visite optimisées, cartographie
  florale par satellite) constituerait un avantage concurrentiel significatif.

  \item \textbf{Connecteur IoT standardisé.}
  Plutôt que de développer du matériel, créer un endpoint d'ingestion standardisé
  (\texttt{/api/v1/telemetry}) avec authentification par token device et schéma de
  données normalisé (poids, température, humidité, acoustique). Cela permettrait
  l'intégration de capteurs existants (BroodMinder, ApisProtect, BEEP, ESP32 DIY)
  sans développer le hardware.

  \item \textbf{Détection acoustique et vision par ordinateur.}
  Les recherches récentes montrent une précision de 85 à 99\,\% pour la détection
  acoustique du varroa et de l'essaimage, ainsi que le comptage automatisé de varroa
  sur planche collante par vision (YOLOv11 Nano, $R^2 = 0{,}99$). Ces modèles
  open-source pourraient être intégrés dans un module d'inspection photo et un
  analyseur acoustique, enrichissant les rapports de visite d'indicateurs automatisés.

  \item \textbf{Jumeau numérique de colonie et crédits carbone.}
  À plus long terme, la modélisation prédictive (jumeaux numériques) permettrait des
  simulations \emph{what-if} sur le traitement du varroa, la nourriture ou la
  migration. Le suivi de la pollinisation de précision et le calcul d'empreinte
  carbone apicole ouvriraient l'accès aux marchés de crédits carbone et aux
  subventions du Pacte vert européen.
\end{itemize}
