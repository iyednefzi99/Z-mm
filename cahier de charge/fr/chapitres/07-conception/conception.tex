...
Séquence & Déroulé de la réalisation d'une visite (Agent, client React, contrôleur REST, service métier, repository, base de données). \\
...
Déploiement & Répartition sur le navigateur, le proxy inverse, le conteneur applicatif, le SGBD, le fournisseur d'identité Keycloak et les capteurs. \\
\bottomrule
\end{xltabular}
...
\node[couche] (p) at (0,4) {Présentation : React et TypeScript};
\node[couche] (c) at (0,3) {Contrôle : Spring MVC (API REST)};
\node[couche] (m) at (0,2) {Métier : services Spring};
\node[couche] (d) at (0,1) {Persistance : Spring Data JPA et jOOQ};
\node[couche] (b) at (0,0) {PostgreSQL (PostGIS, TimescaleDB)};
\node[draw=mielfonce,thick,fill=miel!15,minimum height=8mm,font=\footnotesize,align=center,text width=3.1cm,right=4mm of c] (s) {Ressources i18n\\ConfigZumm.ini\\Contrat OpenAPI};
...
\node[noeud,right=1.6cm of nav] (srv) {Conteneur Zümm\\(Spring Boot)};
\node[noeud,right=1.6cm of srv] (bd) {PostgreSQL\\PostGIS + TimescaleDB};
\node[noeud,below=1cm of srv] (oauth) {Keycloak\\(fédération Google)};
...
(agent, client React, contrôleur REST, service métier, base de données) ainsi que les cas en ligne et
