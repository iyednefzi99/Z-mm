...
i18n par fichier XML & \texttt{messages\_fr/\allowbreak en/\allowbreak ar\allowbreak .properties} + \texttt{MessageSource} & Meme principe d'externalisation des textes, format \texttt{.properties} standard, toujours FR / EN / AR. \\
\texttt{ConfigZumm.ini} & \texttt{application.yml} + \texttt{@Configuration\allowbreak Properties} & Parametrage sans recompilation, injectable par profil. Les seuils et unites deviennent des beans types. \\
...
Web service API XML \texttt{getZumm\allowbreak Honey\allowbreak Actual\allowbreak Quantity} & API REST JSON (\texttt{@GetMapping}) + OpenAPI / Swagger & JSON au lieu de XML, documentation auto-generee. XML reste possible via \texttt{produces = APPLICATION\_XML}. \\
