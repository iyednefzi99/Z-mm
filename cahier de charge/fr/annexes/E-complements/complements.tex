...
sur le nombre de hausses, la capacite en cadres et la productivite, ainsi que l'unicite du
...
\subsection{Table de correspondance du vocabulaire}
Pour garantir la coherence entre les livrables, chaque entite metier porte le
meme concept sous trois formes : le nom du \textbf{dictionnaire de donnees}
(chapitre~5), la \textbf{classe} du diagramme UML et la \textbf{table} du MLD.
Convention d'ecriture des attributs : \texttt{camelCase} dans les classes
(\texttt{nbHausses}), \texttt{snake\_case} dans les tables (\texttt{nb\_hausses}).

\begin{xltabular}{\textwidth}{>{\bfseries}X p{4.4cm} p{4.4cm}}
\caption{Correspondance des noms entre dictionnaire, classes et MLD.}
\label{tab:vocabulaire}\\
\toprule
\textbf{Dictionnaire (metier)} & \textbf{Classe (UML)} & \textbf{Table (MLD)} \\
\midrule
\endfirsthead
\toprule
\textbf{Dictionnaire (metier)} & \textbf{Classe (UML)} & \textbf{Table (MLD)} \\
\midrule
\endhead
Fermier & \texttt{Fermier} & \texttt{FERMIER} \\
Ferme & \texttt{Ferme} & \texttt{FERME} \\
Site & \texttt{Site} & \texttt{SITE} \\
Ruche & \texttt{Ruche} & \texttt{RUCHE} \\
Compartiment (corps / hausse) & \texttt{Compartiment} & \texttt{COMPARTIMENT} \\
Cadre & \texttt{Cadre} & \texttt{CADRE} \\
Agent & \texttt{Agent} & \texttt{AGENT} \\
Planning & \texttt{Planning} & \texttt{PLANNING} \\
Visite & \texttt{Visite} & \texttt{VISITE} \\
Photo & (rapport de visite) & \texttt{PHOTO} \\
Mesure & \texttt{Mesure} & \texttt{MESURE} \\
Recolte & \texttt{Recolte} & \texttt{RECOLTE} \\
Seuil & \texttt{Seuil} & \texttt{SEUIL} \\
\bottomrule
\end{xltabular}
...
\midrule
\endhead
Retard de developpement       & Moyenne & Fort      & Prioriser le noyau CRUD, avec jalons hebdomadaires (S5 a S12). \\
...
Failles applicatives (injection, XSS) & Moyenne & Fort & Requetes parametrees (JPA / jOOQ), echappement automatique de React, revue de securite. \\
...
Les principes ci-dessus se traduisent par les mesures techniques suivantes, par
categorie de donnees. Le \textbf{chiffrement au repos} vise en priorite les
colonnes sensibles (contacts, geolocalisation). La \textbf{pseudonymisation}
dissocie l'identite de l'agent de l'historique agronomique lors de l'anonymisation.

{\small
\begin{xltabular}{\textwidth}{>{\bfseries}p{3.1cm} p{3.3cm} p{2.7cm} X}
\caption{Mesures techniques de protection des donnees (esprit RGPD).}
\label{tab:rgpd}\\
\toprule
\textbf{Donnee} & \textbf{Finalite} & \textbf{Retention} & \textbf{Securite / effacement} \\
\midrule
\endfirsthead
\toprule
\textbf{Donnee} & \textbf{Finalite} & \textbf{Retention} & \textbf{Securite / effacement} \\
...
Mesures capteurs & Analyse de performance & Brutes 1 an, puis agregees & Chiffrement au repos, agregation / anonymisation \\
Journaux d'acces & Securite et imputabilite & 6 a 12 mois & Acces administrateur, purge automatique \\
\bottomrule
\end{xltabular}}

\begin{encadre}
\textbf{Procedure d'effacement :} sur demande d'une personne concernee, le
systeme produit d'abord un export (portabilite CSV / TXT), puis efface ou
anonymise les enregistrements selon la table ci-dessus. Un \textbf{registre des
traitements} et un referent protection des donnees sont tenus pour le projet.
\end{encadre}
...
Integration   & Acces aux donnees (Repository / JPA) & CRUD persiste, requetes de calcul                                   & JUnit + Testcontainers \\
...
substituer un \texttt{RucheRepository} en memoire aux implementations JPA, ce
