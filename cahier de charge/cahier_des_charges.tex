...
Modularité configurable & Activation des modules par profil via ConfigBKS.ini & Moyenne \\
...
Surcharge de fonctions inutiles & Modules activables par profil et par le fichier de configuration ConfigBKS.ini, grâce à l'architecture faiblement couplée. \\
...
Ruche & id, modele & entier, texte & Ruche rattachée à un site. \\
...
Corps ou hausse & id, type & entier, énumération & Compartiment (corps ou hausse). \\
Corps ou hausse & capaciteCadres & entier (1 à 10) & Nombre de slots de cadres. \\
...
Cette valeur est exposée par le service web via \texttt{getBeeHouseHoneyActualQuantity(beeHouseID)}.
\begin{itemize}
...
quantité de miel d'une ruche : \texttt{SELECT SUM(quantiteMiel) FROM recolte WHERE ruche = beeHouseID}.
...
\begin{itemize}
  ...
  \item \textbf{Scénario nominal} : l'application appelle \texttt{getBeeHouseHoneyActualQuantity(beeHouseID)}, le service calcule et retourne la quantité de miel.
...
métier, leurs attributs, la méthode de service \texttt{getBeeHouseHoneyActualQuantity}
...
\node[draw=mielfonce,thick,fill=miel!15,minimum height=8mm,font=\footnotesize,right=3mm of c,xshift=4.7cm] (s) {i18n XML / ConfigBKS.ini / Services web};
...
Configuration    & Paramétrage par fichier texte \texttt{ConfigBKS.ini}. \\
...
API tierce       & Service web XML exposant \texttt{float getBeeHouseHoneyActualQuantity(int beeHouseID)} pour récupérer la quantité de miel d'une ruche. \\
...
ConfigBKS.ini & Paramétrer le système sans recompilation & Module de configuration \\
...
\begin{itemize}
  \item \textbf{Packaging} : l'application est empaquetée sous forme d'archive Java d'entreprise (WAR pour la partie web, EAR si les EJB sont regroupés), incluant les fichiers XML d'internationalisation et le fichier \texttt{ConfigBKS.ini}.
  ...
  \item \textbf{Configuration} : les seuils, unités et modules actifs sont ajustés dans \texttt{ConfigBKS.ini} sans redéploiement.
\end{itemize}
...
TC10 & API miel & Appeler getBeeHouseHoneyActualQuantity & Quantité correcte retournée \\
TC11 & Internationalisation & Basculer FR, EN puis AR & Textes traduits via le fichier XML \\
TC12 & Configuration & Modifier un seuil dans ConfigBKS.ini & Nouveau seuil pris en compte \\
...
\begin{longtable}{>{\bfseries}p{6.5cm} p{7cm}}
\toprule
\textbf{Exigence} & \textbf{Cas de test} \\
\midrule
\endhead
Gestion des entités et contraintes & TC01, TC02, TC03, TC04 \\
Planification et rapports de visite & TC05, TC06 \\
Tableaux de bord et alertes & TC07, TC08, TC09 \\
Service web API & TC10 \\
Internationalisation et configuration & TC11, TC12 \\
Authentification et sécurité & TC13, TC14 \\
Portabilité et mode hors-ligne & TC15, TC16 \\
\bottomrule
\end{longtable}
...
\input{annexe_migration_springboot}

\input{annexe_solid_patterns}
