...
| ~~Flux OIDC toujours non joué en CI~~ | **Soldé le 12/09/2026** : `AuthentificationOidcReelleIT` joue le flux `/bff/connexion` pour de vrai contre un Keycloak Testcontainers (realm de dev importé), pas un `oidcLogin()` fabriqué — et a trouvé deux pièges réels au passage : `CsrfFilter` ne pose le cookie CSRF que sur la première MUTATION rejetée (jamais sur un GET), et le cookie de session `Secure` sans condition exige un client qui parle vraiment TLS (certificat auto-signé jetable, généré par `keytool`). Le flux redirection + PKCE (`oauth2Login()`, client `zumm-frontend`) reste non joué pour de vrai — la PWA ne l'emprunte plus, ADR-009 | SPRINT-11 |
...
| ~~Aucune alerte sur anomalie d'accès~~ | **Soldé le 12/09/2026** : `DetecteurAnomalieAcces` compte les refus RBAC (403) par acteur sur une fenêtre glissante et, au-delà du seuil, engendre une tâche critique et alerte les responsables/admin par courriel | SPRINT-09 |
