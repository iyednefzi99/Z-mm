...
\midrule
Local      & \texttt{feature/*}  & \texttt{http://localhost:8080/zumm} & Développement local (WildFly) \\
...
\node[stage, right=of build] (test)    {TESTS\\ \scriptsize JUnit + Arquillian\\ \scriptsize (13 min)};
...
\midrule
Build & Compilation Maven + packaging WAR/EAR (JDK 17, Maven 3.9+, WildFly)
...
\addlinespace
Tests d'intégration & Arquillian \emph{in-container} + H2
...
Sécurité & Scan des dépendances OWASP
  ...
  & --- & 4 min \\
\addlinespace
Build Docker & Image WildFly + WAR Zümm
...
\begin{itemize}
  \item \textbf{Docker} : \texttt{Dockerfile} applicatif (WildFly + WAR),
...
\path{zumm_scrum_devops_roadmap/03_devops_pipeline/} ;
ce chapitre en
