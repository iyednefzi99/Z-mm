...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/%D8%AA%D8%BA%D8%B7%D9%8A%D8%A9%20%D8%A7%D9%84%D8%AE%D8%A7%D8%AF%D9%85-92.3%25-2E9E3F" alt="تغطية الخادم 92.3%">
...
- تخطيط الجولات، إسناد الأعوان، والمصادقة على الجدول أو رفضه من قِبل مسؤول. تُصدَّر
  الزيارات المُبرمَجة بصيغة iCalendar، ويمكن للعون الاشتراك في أجندته من هاتفه.
- تتبّع العنوان البريدي للمنحل وموارده الرحيقية، وتاريخ مواقعه (الترحال)، وعمليات
  التقسيم والتقاط الطرود.
- العمل **دون اتصال**: يحتفظ التطبيق بما أُدخل ويعيد إرساله عند عودة الشبكة. ويُؤخذ
  المنحل **صراحةً** قبل الصعود إليه — لقطة **مؤرَّخة من الخادم**، تنتهي صلاحيتها بعد
  أربعة عشر يومًا — وأيّ إدخال يُرفض عند إعادة الإرسال ينتقل إلى **الحجر** مع سببه، بدل
  أن يختفي.
- استئناف مسوّدة زيارة من جهاز آخر: يبقى محتواها **مُبهَمًا** بالنسبة إلى الخادم ولا
  يدخل أيّ سجلّ ما لم يُعتمد.
- تركيب بطاقة معاينة خاصّة انطلاقًا من مرجع **مغلق** من أربع وأربعين نقطة ملاحظة —
  تُفعَّل خانات موجودة، ولا تُخترع جديدة، وإلّا لم يعد شيء قابلًا للعدّ بين الضيعات.
- إملاء ملاحظة صوتيًا: يجري النسخ **على الجهاز**، وترفض الواجهة الانطلاق حيثما كان
  المتصفّح سيرسل الصوت إلى خدمة طرف ثالث.
- فتح بطاقة الخلية بمسح رمز **QR** أو شريحة **NFC**، مع لوحة ملصقات قابلة للطباعة.
...
- جعل الوصفة البيطرية **قابلة للتحقّق**: يُرفَق مسحها الضوئي بالعلاج، ويحتفظ العلاج
  بنسخته **الخاصّة** من فترة الأمان — فالنشرة هي المرجع، لا الفهرس.
- تجميع أوراق مراقبة الفلاحة العضوية مع **تسمية ما لا يستطيع Zümm التحقّق منه**: مصدر
  السكّريات، ومصدر الشمع، ووضعية العقار تخرج «قيد الإثبات»، لا «مطابِقة» أبدًا.

**التربية والوراثة**

- حفظ نسب الملكات — ملكة، وأمّها، وسلالتها — وقراءته على هيئة شجرة.
- متابعة سلاسل التطعيم وسجلّ التربية النظامي بصيغة PDF.
- مقارنة السلالات على خمسة معايير، كلٌّ بوحدته وبعدد ملاحظاته. **بلا علامة إجمالية**:
  فجمع الطبع مع المردود لا يعني شيئًا.
...
- تسجيل عمليات الجني — العسل، وكذلك حبوب اللقاح والعكبر والغذاء الملكي والشمع والطرود
  والملكات، كلٌّ بوحدته —، تكوين دفعات التعليب، وتتبّع الدفعة رجوعًا إلى الخلايا التي
  أتت منها.
- توليد نصّ البطاقة القانونية للدفعة ورمز QR للتتبّع.
- مقارنة موسم بالذي سبقه على **سنوات تقويمية**، حيث تنزلق كلّ التجميعات الأخرى في
  المنتج على اثني عشر شهرًا متحرّكة.
- مسك جرد العتاد وخطّة صيانته، ومخزون بعتبات إعادة الطلب، ومحاسبة تعطي المردودية
  **لكلّ خلية**. تبقى المصاريف غير المُسنَدة كاملةً وعلى حدة: فمفتاح توزيع مُخترَع يعطي
  رقمًا أجمل وأقلّ صدقًا.
- تصدير السجلّات كاملةً — أربعة عشر موردًا، بصيغ CSV وTXT وXLSX — وإخراج الحصيلة
  السنوية بصيغة PDF.
...
- **إنذار ضدّ السرقة** مبنيّ على سلسلة الأوزان المُستوعَبة أصلًا: الهبوط جنيٌ إن سُجّل
  جنيٌ في ذلك اليوم، وسرقةٌ فيما عدا ذلك. فبدون هذا التحقّق، سيوقظ أوّل فيض عسلي
  الإنذارَ على المنحل بأكمله.
- قراءة مستشعر **مباشرةً عبر Bluetooth** من المتصفّح، على ملمح SIG المعياري — دون أيّ
  فكّ ترميز خاصّ مُخمَّن.
- الوزن عاسلةً عاسلة، ومشاركة تدفّق قياسات خارج الضيعة برمز محصور في خلية واحدة، ولا
  يحمل أيّ موقع.
- يولّد محرّك قواعد المهامَّ الواجبة؛ أمّا مؤشّرا صحّة الطائفة وخطر التطريد فيُحسبان
  **عند القراءة**، ولا يُخزَّنان أبدًا.

**محيط المنحل**

- إيداع طبقة **استعمال الأرض** الخاصّة بالضيعة، وقراءة المساحات حسب نوع الغطاء داخل
  نطاق السروح، ودورتها من سنة إلى أخرى، والمسافة إلى أقرب قطعة مزروعة. المعطى
  **يُستقبَل ولا يُستعلَم عنه** أبدًا: فسؤال خدمة طرف ثالث يعني إرسال موقع المنحل إليها.
- تدوين الإزهار **المُلاحَظ** ومقابلته بالتقويم المُصرَّح به، مع الفارق بالأيّام —
  فالمُصرَّح يتوقّع، والمُلاحَظ يُثبِت.
- قطع حركة الخادم الصادرة وحركة بلاطات الخريطة كلٌّ على حدة: **مفتاحان**، لأنّ هناك
  حركتين، ومفتاح واحد سيوهم بشبكة صامتة وهي صامتة إلى النصف فقط.
...
| **JaCoCo** | تغطية مدمجة للوحدات والتكامل، بحدّين مانعين عند 92٪ (تعليمات) و80٪ (فروع). |
...
│   └── src/
│       ├── vues/                 42 screens: 23 console, 10 public, sign-in, account, errors
│       ├── ui/                   modal, toasts, SVG charts
│       ├── api/                  openapi.json + generated types
│       ├── auth/  routage/       BFF session, in-house router
│       ├── offline/              replay queue + quarantine
│       ├── terrain/  carnet/     offline take-along, inspection sheet
│       ├── elevage/              queen lineage tree (SVG)
│       ├── environnement/        cover areas, observed bloom
│       ├── capteurs/  voix/      SIG Bluetooth, on-device dictation
│       ├── local/                embedded EWMA
...
**استثناءان اثنان، لا أكثر.** يستجيب `GET /api/calendrier/{jeton}.ics` (الاشتراك في
الأجندة الخاصّة) و`GET /api/flux/{jeton}` (مشاركة تدفّق قياسات) دون جلسة. ويحمل كلاهما
رمزًا من 256 بتّة لا يُخزَّن إلّا **ببصمة SHA-256**، وتنتهي صلاحيته، ويمكن إبطاله،
ويعرض آخر استعمال له، ولا يُعيد **أيّ موقع**. ولا يخضع أيٌّ منهما لتحديد معدّل الطلبات:
وهو ما يجب معالجته قبل أيّ فتح واسع.
...
| `POST` | `/api/traitements/lot` | أي دور وظيفي | خلايا + علاج | **معاملة لكلّ خلية**: تُسمّى حالات الرفض بأسبابها، وتصمد بقية الكتابات |
| `GET` | `/api/ruchers/{id}/emport` | أي دور وظيفي | — | لقطة منحل للعمل دون اتصال، **مؤرَّخة من الخادم**، تنتهي بعد 14 يومًا |
| `GET` | `/api/reines/{id}/lignee` | أي دور وظيفي | — | نسب ملكة صعودًا ونزولًا |
| `GET` | `/api/environnement/sites/{id}/couvert` | أي دور وظيفي | `millesime` | المساحات حسب فئة الغطاء داخل نطاق السروح، مع حصّة الدائرة الموصوفة فعلًا |
| `GET` | `/api/environnement/sites/{id}/rotation` | أي دور وظيفي | — | المساحات نفسها، سنةً بسنة |
| `GET` | `/api/briefing` | أي دور وظيفي | — | ملخّص اليوم، **بلا نموذج لغوي**: أربعة سجلّات مقروءة، كلّ سطر يستشهد بما يستند إليه |
| `GET` | `/api/calendrier/{jeton}.ics` | **بلا** (رمز) | — | أجندة عون واحد، بلا أيّ موقع |
| `GET` | `/api/flux/{jeton}` | **بلا** (رمز) | — | تدفّق قياسات خليّة **واحدة**، بلا أيّ موقع |
...
**العمل دون اتصال أخذٌ للمعطى، لا تخزينٌ مؤقّت له** ([ADR-012](roadmap/operationnel/06_decisions/ADR-012-hors-ligne-selectif.md)).
لا يخزّن عامل الخدمة (service worker) **أيّ** استجابة من الواجهة البرمجية. يُؤخذ المنحل
عند الطلب — محدودًا، مؤرَّخًا من الخادم، وقابلًا للانتهاء؛ وتبقى القياسات والطقس والمواقع
الدقيقة خارجًا. وإضافة `/api` إلى `runtimeCaching` تراجعٌ إلى الوراء، لا اختصار.

**الذكاء الاصطناعي يعمل على الجهاز، أو لا يعمل** ([ADR-013](roadmap/operationnel/06_decisions/ADR-013-ou-tourne-l-ia.md)).
لا ينطلق الإملاء إلّا إذا أعلن المتصفّح أنّه ينسخ محلّيًا. ولا يمرّ ملخّص اليوم عبر **أيّ**
نموذج لغوي: يقرأ أربعة سجلّات ويستشهد بما تستند إليه كلّ سطر. فجملة مولَّدة تُقرأ أفضل
ويُتحقَّق منها أسوأ — وستقتضي إرسال تاريخ الضيعة إلى الخارج.

**لا نشتري عتادًا** ([ADR-014](roadmap/operationnel/06_decisions/ADR-014-capteurs-du-commerce.md)).
يقتصر Bluetooth على ملمح SIG المعياري، ومعرّفاته ووحداته منشورة. أمّا كتابة فاكّ ترميز
لإطار خاصّ بمصنّع لا يملك أحدٌ هنا جهازه فتعني تخمين بنية معطيات: مظهرُ الدعم دون
موثوقيته.

**معطى المحيط يُستقبَل ولا يُستعلَم عنه** ([ADR-015](roadmap/operationnel/06_decisions/ADR-015-occupation-du-sol.md)).
فالاستعلام المباشر عن خدمة لاستعمال الأرض يعني إرسال موقع المنحل إليها — وهو ما تُخفيه
`PolitiquePositions` وما يقطعه النمط المحلّي بالنسبة إلى بلاطات الخريطة. تُودِع الضيعة
طبقتها الخاصّة؛ ويترجمها المُدخِل إلى تصنيف **مغلق** من عشر فئات، وأيّ فئة مجهولة تُفشل
الإيداع **بأكمله** بدل أن تُنتج مساحات خاطئة يبلغ مجموعها مئة بالمئة رغم ذلك.

**رفض ما لا يمكن التحقّق منه.** رُفضت ثلاث ميزات منتظَرة مع تدوين أسبابها: التحليل
الصوتي داخل الخلية، وعدّ حبوب اللقاح، ومهايئ الأطر الخاصّة. كلٌّ منها سينتج رقمًا لا
يستطيع النحّال التثبّت منه إلّا بفتح الخلية. وهو المنطق نفسه الذي يمنع العلامة الوراثية
الإجمالية وحساب ارتباط الصحّة بالغطاء النباتي على عشرة مناحل.
...
أرقام مقروءة من مخرجات المجموعات نفسها يوم 2026-09-06، لا منقولة:

| المجموعة | الحجم | الأدوات |
|---|---|---|
| الخادم — وحدات | **642** اختبارًا، 0 إخفاق، 0 متجاوَز | JUnit 5، Mockito |
| الخادم — تكامل | **242** اختبارًا في 34 صنفًا، 0 إخفاق، **0 متجاوَز** | Testcontainers على PostgreSQL/PostGIS/TimescaleDB حقيقي |
| الخادم — التغطية | **92.3٪** تعليمات، 92.5٪ أسطر، **80.1٪** فروع | JaCoCo، حملات مدمجة، حدّان **مانعان** عند 92٪ (تعليمات) و80٪ (فروع) |
...
**اختباران كُتبا لميزة، فوجدا ميزةً أخرى معطوبة.** خمسة مفاتيح خارجية أُضيفت بين
الترحيلتين `V20` و`V27` كانت تكتب `ON DELETE SET NULL` دون تسمية عمودها: فكانت
PostgreSQL تُفرِغ `tenant_id` أيضًا، وكان حذف خليّة تحمل مصروفًا يفشل بـ `500`. كما أنّ
`GET /api/mesures/alertes` لم يكن ينجح **إلّا على قائمة فارغة** منذ SPRINT-06 — إذ كانت
القراءة تجري خارج أيّ معاملة والاقتران `LAZY`. ولم يستدعِ أيّ اختبار هذا المسار قطّ مع
تنبيه مفتوح. فمسارٌ يغطّيه اختبارٌ لا يضعه أبدًا في الحالة المُهمّة هو مسار غير مُغطّى.
...
- **تغطية الفروع عند 80.1٪**، مقابل 92.3٪ للتعليمات: مسارات الخطأ تبقى أقلّ تغطيةً
  من الاسمية. كانت 63.0٪ يوم 2026-09-05، فرفعتها ثلاث عشرة جولة اختبارات موجَّهة سبعَ
  عشرة نقطة، وتبعها الحدّ المانع من 60٪ إلى 80٪. وهو سقّاطة ضدّ التراجع لا هدف:
  الهامش يُستعاد باختبار القطع **الخالصة** بلا قاعدة بيانات، لا بخفض الحدّ أبدًا.
...
- **كشف الشذوذ**: تبقى خدمة الذكاء الاصطناعي تقييمًا إحصائيًا. والرجوع المحلّي إلى EWMA
  أخشنُ منها، وهو الآن **مُنفَّذ مرّتين** — بلغة Java في الخادم وبـ TypeScript في
  المتصفّح، من أجل الكشف على الجهاز. ويثبّت الاثنين اختباران يثبّتان الأرقام نفسها على
  السلسلة نفسها: فمسّ أحدهما دون الآخر يُفشل إحدى الحملتين.
- **لا تحديد لمعدّل الطلبات** على المسارين العموميين ذَوَي الرمز. فهما محدودان وتنتهي
  صلاحيتهما ولا يحملان أيّ موقع، لكن لا شيء يكبح تكرار النداء — وهو ما يجب معالجته قبل
  أيّ فتح واسع.
- **النمط المحلّي يقطع حركةً واحدة في كلّ مرّة**: فضبط الخادم لا يستطيع شيئًا تجاه بلاطة
  خريطة يطلبها المتصفّح، والعكس بالعكس. المفتاحان موجودان، والواجهة تقول ذلك بدل أن تعد
  بصمتٍ لا تستطيع الوفاء به.
- **مرجع استعمال الأرض غير مُرفَق**: تُودِع الضيعة طبقتها الخاصّة. وتلك هي الكلفة
  المقبولة لعدم الذهاب إلى الخارج من تلقاء النفس.
